#!/usr/bin/env python3
"""
ЭТАП 14: СТРАТЕГИЯ С УЧЁТОМ КАЛИБРОВКИ
- ТБ только при p > 0.75 (избегаем зоны 0.70-0.75)
- ТМ только при p < 0.35 (используем зону 0.35-0.40)
- Калибровочная поправка для проблемных зон
"""
import pandas as pd
import numpy as np
...
print("=" * 55)
print("🚀 ЭТАП 14: СТРАТЕГИЯ С УЧЁТОМ КАЛИБРОВКИ")
print("=" * 55)
...
def calibrate_prob(p):
    """Корректирует вероятность на основе калибровочных данных"""
    if p > 0.70 and p <= 0.75:
        return p - 0.13  # Модель переоценивает на 13 п.п.
    elif p >= 0.35 and p < 0.40:
        return p + 0.14  # Модель недооценивает на 14 п.п.
    elif p > 0.65 and p <= 0.70:
        return p - 0.02  # Небольшая переоценка
    elif p >= 0.60 and p <= 0.65:
        return p + 0.06  # Небольшая недооценка
    return p  # Остальные диапазоны откалиброваны

# ============================================================
# ТЕСТИРУЕМ ТРИ СТРАТЕГИИ
# ============================================================
configs = [
    # (название, порог ТБ, порог ТМ, использовать калибровку)
    ('Без калибровки (ТБ>0.70, ТМ<0.30)', 0.70, 0.30, False),
    ('Без калибровки (ТБ>0.65, ТМ<0.35)', 0.65, 0.35, False),
    ('С калибровкой (ТБ>0.75, ТМ<0.35)', 0.75, 0.35, True),
    ('С калибровкой (ТБ>0.70, ТМ<0.30) + поправка', 0.70, 0.30, True),
]

print(f"\n{'='*80}")
print(f"{'Стратегия':<45} {'Ставок':<7} {'Банк':<10} {'ROI':<9} {'Hit':<8}")
print(f"{'-'*80}")

for name, tb_thresh, tm_thresh, use_cal in configs:
    bank = 10000
    bets = wins = 0
    total_ev_sum = 0
    ...
    for i, (_, row) in enumerate(test.iterrows()):
        ...
        p_use = calibrate_prob(p) if use_cal else p

        # ТБ
        if p_use > tb_thresh and row['odds_over'] > 0:
            ev = p_use * row['odds_over'] - 1
            if ev > 0.05:
                bets += 1; total_ev_sum += ev
                bank += stake * (row['odds_over'] - 1) if total > 2.5 else -stake
                wins += 1 if total > 2.5 else 0

        # ТМ
        elif p_use < tm_thresh and row['odds_under'] > 0:
            ev = (1 - p_use) * row['odds_under'] - 1
            if ev > 0.05:
                bets += 1; total_ev_sum += ev
    ...
    avg_ev = total_ev_sum / bets * 100 if bets > 0 else 0
    print(f"{name:<45} {bets:<7} {bank:<10.0f} {roi:<+8.2f}% {hit:<7.1f}% (ср.EV: {avg_ev:+.1f}%)")
